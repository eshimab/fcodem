...
%
% RECENT HISTORY 6/4/2020, 10:57:24 AM - Eric Shimabukuro
% Two Changes:
% 1. Error reprting for FCS file that do not load correctly was changed from a msgbox to an sprintf
% command that prints to the MATLAB workspace. These changes can be found around lines:
% 118, 152, 199, 293.
% 2. The inner function get_mnemonic_value was altered to allow processing for
% Channel names with the '/' character in them. That character is used as the
% parameter separator in the FCS files as well, causing channel names to record
% incorrectly. For Example: "692/40 R (488)" would come out as "692" because the
% / would be counted as a parameter separator, and not a character within the channel name.
% fca_readfcs.m would search for the parameter strings (ie $P4N) and so this did not affect
% parameter recording or other values. Part of the metadata would essentially be skipped:
% "$P4N/692//40 R (488)/$P4E/" would be considered "$P4N/692/$P4E/" The changed function finds
% escaped "/" characters and extracts the entire channel name.
% In the original file, the get_mnemonic_value lines were 454-457.
% They are now: 484-504.
% MORE: % 7/21/2020, 10:25:40 AM - Eric Shimabukuro
% Added lines 295-330, added new fcshdr fields to show timestep, sampleid, trigger
% channel and level, and then comments that I have used to enter in metadata like the config file,
% start time, etc.
%
%
% History
...
if nargin == 0
    ...
    if size(filecheck,1) == 0 || size(filecheck,1) >1
      % ----------------- Changed msgbox to sprintf ----------------------
      % 5/6/2020
        % hm = msgbox([filename,': The FCS file or the source directory does not exist!'], ...
            % 'FCS reading info','warn');
        sprintf(['-------------------- ' filename ' FAILED TO PARSE --------------------- \n The FCS file or the source directory does not exist!'])
        fcsdat = []; fcshdr = []; fcsdatscaled= []; fcsdat_comp= [];
        return;
    end
end
...
if strcmp(fcsheader_type,'FCS1.0')
  ...
  % 5/6/2020
    ...
    sprintf(['-------------------- ' filename ' FAILED TO PARSE --------------------- \n FCS 1.0 file type is not supported!'])
    fcsdat = []; fcshdr = []; fcsdatscaled= []; fcsdat_comp= [];
    fclose(fid);
    return;
elseif  strcmp(fcsheader_type,'FCS2.0') || strcmp(fcsheader_type,'FCS3.0')  || strcmp(fcsheader_type,'FCS3.1') % FCS2.0 or FCS3.0 or FCS3.1 types
    ...
    if mnemonic_separator == '@';% WinMDI
      ...
      % 5/6/2020
        ...
        sprintf(['-------------------- ' filename ' FAILED TO PARSE --------------------- \n The file can not be read (Unsupported FCS type: WinMDI histogram file)'])
        fcsdat = []; fcshdr = [];fcsdatscaled= []; fcsdat_comp= [];
        fclose(fid);
        return;
    end
...
%
    ...
    % ----------------- ADDED 7/21/2020 BY EMS -------------------
    fcshdr.cells = get_mnemonic_value('$CELLS',fcsheader_main, mnemonic_separator); % cell type
    fcshdr.timestep = get_mnemonic_value('$TIMESTEP',fcsheader_main, mnemonic_separator);
    fcshdr.sampleid = get_mnemonic_value('$SMNO',fcsheader_main, mnemonic_separator); % sample_id
    triggerString = get_mnemonic_value('$TR',fcsheader_main, mnemonic_separator);
    if ~isempty(triggerString)
      triggerStringSplit = strsplit(triggerString,',');
      trigchn = triggerStringSplit{1};
      triglvl = triggerStringSplit{2};
      fcshdr.trigchn = trigchn;
      fcshdr.triglvl = triglvl;
    end
    % ----------------- Get comments from FCS file -------------------
    commentStruct = struct;
    commentString = get_mnemonic_value('$COM',fcsheader_main, mnemonic_separator);
    commentStruct.string = commentString;
    if ~isempty(commentString)
      commentLine = strip(commentString);
      % I use the | to separate out parts of the comments
      commentParts = cellfun(@(x) strip(x),strsplit(commentLine,'|'),'UniformOutput',false);
      commentParts = commentParts(~cellfun('isempty',commentParts));
      for i = 1:length(commentParts)
        part = commentParts{i};
        if contains(part,'XML','IgnoreCase',true)
          commentStruct.config = part;
        elseif length(part) == 5 && contains(part,':')
          commentStruct.start = part;
        elseif contains(part,'BEADS','IgnoreCase',true) | contains(part,'BDS','IgnoreCase',true)
          commentStruct.beads = part;
        elseif contains(part,'SYBR','IgnoreCase',true) & ~contains(part,'XML','IgnoreCase',true)
          commentStruct.sybr = part;
        end
      end
      commentStruct.string = commentString;
      fcshdr.comments = commentStruct;
    end
    % ----------------- END OF 7/21/2020 ADDITION BY EMS -------------------

else
  % ----------------- Changed msgbox to sprintf ----------------------
  % 5/6/2020
    % hm = msgbox([FileName,': The file can not be read (Unsupported FCS type)'],'FCS reading info','warn');
    sprintf(['-------------------- ' filename ' FAILED TO PARSE --------------------- \n The file can not be read (Unsupported FCS type)'])
    fcsdat = []; fcshdr = []; fcsdatscaled= []; fcsdat_comp= [];
    fclose(fid);
    return;
end
...
if strcmp(mnemonic_separator,'\')  || strcmp(mnemonic_separator,'!') ...
    ...
    if length(mnemonic_startpos > 1)
      mnemonic_startpos = mnemonic_startpos(1);
      mnemonic_stoppos = mnemonic_stoppos(1);
    end
    % in a few fcs files, the $SMNO param was twice (see sHOT-220-2-2-01.fcs)
    % mneval = char(fcsheader(mnemonic_stoppos+1:next_slash-1)');
    % ----------------- ADDED: Escaped / Characters  -------------------
    all_slashes = findstr(char(fcsheader'),mnemonic_separator); % index of all separators
    esc_slashes = findstr(char(fcsheader'),['/' mnemonic_separator]); % index of escaped separators
    for j = 1:length(esc_slashes)
      slashind = esc_slashes(j); % get index of the first / in escaped chars
      % Removing the index locations where there are two // in a row.
      all_slashes(all_slashes==slashind) = []; % Removing index of escaping /
      all_slashes(all_slashes==slashind+1) = [];
    end
    paramstart = mnemonic_stoppos+1;
    paramend = all_slashes(find(all_slashes > mnemonic_stoppos,1,'first'))-1; % Next index
    param = char(fcsheader(paramstart:paramend))'; % param string
    escapestr = ['/' mnemonic_separator]; % escaped string
    param = strrep(param,escapestr,mnemonic_separator);
    % ----------------- End Added -------------------
    mneval = param;
